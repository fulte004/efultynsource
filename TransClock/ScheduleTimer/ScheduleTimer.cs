...
namespace Schedule
{
    ...
    public class ScheduleTimer : ScheduleTimerBase
    {
        ...
        public ScheduleTimer(ScheduleConfiguration configuration) : base() {
            AddEventsFromScheduleConfiguration(configuration);
        }
        /// <summary>
        ...
        public void AddScheduledEvent(ScheduledEvent scheduledEvent) {
            if (scheduledEvent.ScheduledTime != null) {
                AddEvent(scheduledEvent.ScheduledTime);
                return;
            }
            if (scheduledEvent.SimpleInterval != null) {
                AddEvent(scheduledEvent.SimpleInterval);
                return;
            }
            if (scheduledEvent.SingleEvent != null) {
                AddEvent(scheduledEvent.SingleEvent);
            }
        }

        public void AddEventsFromScheduleConfiguration(ScheduleConfiguration config) {
            foreach (ScheduledEvent scheduledEvent in config.ScheduledEvents) {
