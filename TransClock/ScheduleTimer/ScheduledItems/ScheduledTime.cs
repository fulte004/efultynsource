...
{
    [Serializable]
    public enum EventTimeIntervalEnum
    {
        BySecond = 1,
        ByMinute = 2,
        Hourly = 3,
    ...
    public class ScheduledTime : IScheduledItem
    {
        ...
        public ScheduledTime(EventTimeIntervalEnum Base, TimeSpan Offset) {
            EventTimeInterval = Base;
            TimeOffset = Offset;
        }
        ...
        public ScheduledTime(string StrBase, string StrOffset) {
            ...
            EventTimeInterval = (EventTimeIntervalEnum)Enum.Parse(typeof(EventTimeIntervalEnum), StrBase, true);
            Init(StrOffset);
        }
        ...
        public DateTime NextRunTime(DateTime time, bool AllowExact) {
            DateTime NextRun = LastSyncForTime(time) + TimeOffset;
        ...
        private DateTime LastSyncForTime(DateTime time) {
            switch (EventTimeInterval) {
                case EventTimeIntervalEnum.BySecond:
                    return new DateTime(time.Year, time.Month, time.Day, time.Hour, time.Minute, time.Second);
                case EventTimeIntervalEnum.ByMinute:
                    return new DateTime(time.Year, time.Month, time.Day, time.Hour, time.Minute, 0);
                case EventTimeIntervalEnum.Hourly:
                    return new DateTime(time.Year, time.Month, time.Day, time.Hour, 0, 0);
                case EventTimeIntervalEnum.Daily:
                    return new DateTime(time.Year, time.Month, time.Day);
                case EventTimeIntervalEnum.Weekly:
                    return (new DateTime(time.Year, time.Month, time.Day)).AddDays(-(int)time.DayOfWeek);
                case EventTimeIntervalEnum.Monthly:
        ...
        private void Init(string StrOffset) {
            switch (EventTimeInterval) {
                case EventTimeIntervalEnum.BySecond:
                    TimeOffset = new TimeSpan(0, 0, 0, 0, int.Parse(StrOffset));
                    break;
                case EventTimeIntervalEnum.ByMinute:
                    string[] ArrMinute = StrOffset.Split(',');
                    TimeOffset = new TimeSpan(0, 0, 0, ArrayAccess(ArrMinute, 0), ArrayAccess(ArrMinute, 1));
                    break;
                case EventTimeIntervalEnum.Hourly:
                    string[] ArrHour = StrOffset.Split(',');
                    TimeOffset = new TimeSpan(0, 0, ArrayAccess(ArrHour, 0), ArrayAccess(ArrHour, 1), ArrayAccess(ArrHour, 2));
                    break;
                case EventTimeIntervalEnum.Daily:
                    DateTime Daytime = DateTime.Parse(StrOffset);
                    TimeOffset = new TimeSpan(0, Daytime.Hour, Daytime.Minute, Daytime.Second, Daytime.Millisecond);
                    break;
                case EventTimeIntervalEnum.Weekly:
                    ...
                    TimeOffset = new TimeSpan(int.Parse(ArrWeek[0]), WeekTime.Hour, WeekTime.Minute, WeekTime.Second, WeekTime.Millisecond);
                    break;
                case EventTimeIntervalEnum.Monthly:
                    ...
                    TimeOffset = new TimeSpan(int.Parse(ArrMonth[0]) - 1, MonthTime.Hour, MonthTime.Minute, MonthTime.Second, MonthTime.Millisecond);
                    break;
                default:
                    throw new Exception("Invalid base specified for timer.");
            }
        ...
        /// <summary>
        /// Gets or sets the event time interval.
        /// </summary>
        [XmlAttribute]
        public EventTimeIntervalEnum EventTimeInterval { get; set; }

        /// <summary>
        /// Gets or sets the raw TimeSpan value of the Time Offset.
        /// </summary>
        [XmlIgnore]
        public TimeSpan TimeOffset { get; set; }

        /// <summary>
        /// Serializable time offset.
        /// </summary>
        [XmlAttribute("TimeOffset")]
        public string SerializableTimeOffset {
            get { return TimeOffset.ToString(); }
            set { TimeOffset = TimeSpan.Parse(value); }
        }
